...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'fake_citizen_id' AND schema_id = SCHEMA_ID('core'))
BEGIN
    CREATE TABLE core.fake_citizen_id
    (
        citizen_id       INT          NOT NULL,           -- real key
        fake_citizen_id  BIGINT       NOT NULL,           -- surrogate

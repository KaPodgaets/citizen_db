...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'breath_troubles' AND schema_id = SCHEMA_ID('stage'))
BEGIN
    CREATE TABLE stage.breath_troubles (
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'breath_troubles' AND schema_id = SCHEMA_ID('core'))
BEGIN
    CREATE TABLE core.breath_troubles (
        citizen_id INT NOT NULL,
        phone_number NVARCHAR(200) NULL,
        phone_number_contact_person NVARCHAR(200) NULL,

...
def main():
    ...
    with engine.begin() as conn:
        ...
        print('[publish]: Get one mega table by merge')
        
        print('[publish]: Clean table with pandas')

        print('[publish]: Delete previous data')
        delete_all_rows_from_mart_table(conn)
        
        print('[publish]: insert new data')
...
def check_mart_table_exists(conn):
    ...
    if table_exists:
        ...
        raise Exception('[publish][check_mart_table_exists]: Table mart.citizens does not exist.')

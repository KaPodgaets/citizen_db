...
def main(dataset: str, period: str, version: int):
    ...
    with engine.begin() as conn:
        ...
        if stage_df.empty:
            print(f"[WAR]: No data found in stage.{staging_table_name} for period {period}. Nothing to transform.")
            return
        ...
        full_df['_data_period'] = pd.to_datetime(full_df['_data_period'], format='%Y-%m')
        ...
        existing_cols_to_drop = [col for col in metadata_cols_to_drop if col in stage_df.columns]

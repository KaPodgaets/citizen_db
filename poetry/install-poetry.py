...
def data_dir() -> Path:
    if os.getenv("POETRY_HOME"):
        return Path(os.getenv("POETRY_HOME")).expanduser() # type: ignore
...
def bin_dir() -> Path:
    if os.getenv("POETRY_HOME"):
        return Path(os.getenv("POETRY_HOME")).expanduser() / "bin" # type: ignore
...
class Installer:
    ...
    def make_env(self, version: str) -> VirtualEnvironment: # type: ignore
        ...
        try:
            ...
            yield VirtualEnvironment.make(env_path) # type: ignore
    ...
    def get_version(self):
        ...
        def _compare_versions(x, y):
            ...
            vx = (*tuple(int(p) for p in mx.groups()[:3]), mx.group(5)) # type: ignore
            vy = (*tuple(int(p) for p in my.groups()[:3]), my.group(5)) # type: ignore
        ...
        if not version:
            for release in reversed(releases):
                ...
                if m.group(5) and not self.allows_prereleases(): # type: ignore
                    continue
